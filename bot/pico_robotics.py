...
class _I2CBoard:
    ...
    def initPCA(self):
        ...
        for r in (0xfa, 0xfb, 0xfc, 0xfd):
            self.i2c.writeto_mem(self.CHIP_ADDRESS, r, b'\x00')
        self.i2c.writeto_mem(self.CHIP_ADDRESS, 0x00, b'\x01')
        utime.sleep_us(500)
    ...
    def release(self, servo):
        reg = self.SRV_REG_BASE + (servo - 1) * self.REG_OFFSET
        self.i2c.writeto_mem(self.CHIP_ADDRESS, reg + 1, bytes([0x10]))
